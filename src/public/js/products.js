...
window.addEventListener("load", async (event) => {
  ...
  if (userEmailLogeado || userNameLogeado) {
    console.log("usuario logueado...");
    Swal.fire(
      "Bienvenido!!!",
      `Iniciaste sesión como: ${userEmailLogeado}`,
      "success"
    );
  } else {
...
btnCerrarSesion.addEventListener("click", async (event) => {
  ...
  if (data.status === "success") {
    Swal.fire("Se cerro sesion con exito!!!", "", "info");
    window.location.replace("/login");
  } else {
    Swal.fire("No se pudo cerrar sesión !!!", "", "warning");
  }
});
